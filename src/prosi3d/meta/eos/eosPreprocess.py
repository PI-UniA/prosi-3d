...
def vecTiming_hdf(total,lay=1,debug=0,readonly=0, changeExposure=1):
    ...
    while lay <= total:
        ...
        df['v'] = EosPreprocessor.__setSpeed(df, df['exposureType'], df['partId'])
        ...
        if len(df)%2 == 1:
            ...
            df.v.iloc[0::2] = EosPreprocessor.__jumpAcceleration(df.dis.iloc[0::2])
        # even line count: start with first line
        else:
            # except first layer
            df.v.iloc[1::2] = EosPreprocessor.__jumpAcceleration(df.dis.iloc[1::2])
...
def setSpeed(df, expType, partId):
    ...
    speed = EosPreprocessor.__parByCsv(parfile)
